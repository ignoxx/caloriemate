...
export function OnboardingModal({ onComplete }: OnboardingModalProps) {
  ...
  const handleComplete = () => {
    if (formData.customCalories && formData.customProtein) {
      onComplete({
        calories: Number.parseInt(formData.customCalories),
        protein: Number.parseInt(formData.customProtein),
        weight: Number.parseInt(formData.weight),
        age: Number.parseInt(formData.age),
      })
    } else {
      onComplete(calculateGoals())
    }
  }
          ...
          {step === 3 && (
            <>
              <div className="bg-blue-50 dark:bg-blue-950/50 p-4 rounded-lg space-y-2">
                <h3 className="font-medium text-blue-900 dark:text-blue-100">Calculated Targets</h3>
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  Calories: {calculateGoals().calories} kcal/day
                </p>
                <p className="text-sm text-blue-700 dark:text-blue-300">Protein: {calculateGoals().protein}g/day</p>
              </div>

              <div className="space-y-4">
                <p className="text-sm text-gray-600">Or set custom targets:</p>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleInputChange("customCalories", e.target.value)}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleInputChange("customProtein", e.target.value)}
                  />
                </div>
              </div>
